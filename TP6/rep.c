...
int main(int argc, char *argv[])
{
	...
	if (init_rep(&repertoire) <0) return 0;
...
int saisie_enreg(Enregistrement *enr)
{
	...
	printf("Nom :");
	l = saisie_chaine(tmp, MAX_SAISIE);
	if (l < 0)
		return ERROR;
	/* chaine vide ? */
	if (l == 0)
	...
	strncpy(enr->nom, tmp, (_countof(enr->nom)-1));
	...
	strncpy(enr->prenom, tmp, (_countof(enr->prenom)-1));
	...
	strncpy(enr->tel, tmp, (_countof(enr->tel)-1));
...
int rechercher_tel(Repertoire *rep, char tel[], int ind)
{
	...
	strncpy(tmp_tel, tel, (_countof(tmp_tel)-1));
	...
	while ((currentElement != NULL) && (!trouve)) {
		strncpy(tmp_tel2, currentElement->pers.tel,(_countof(tmp_tel2)-1));
		compact(tmp_tel2);
	...
	while ((i <= ind_fin) && (!trouve))
	{
		...
		strncpy(tmp_tel2, rep->tab[i].tel, (_countof(tmp_tel2)-1));
		compact(tmp_tel2);
		if (strcmp(tmp_tel, tmp_tel2) == 0)
			trouve = true;
		else
		...
		case 'N':
			...
			do
			{
				...
				else
				{
					...
					else printf("%s",mess1);						/* Pas d'occurence trouvee la premiere fois*/
